...
import { Shield, CheckCircle, Database, Server, Cloud, Lock, ArrowRight, ArrowLeft, LayoutDashboard, Zap } from 'lucide-react';
import { Approvals } from './Approvals';
import './App.css';

type Step = 1 | 2 | 3;
type ConnectorType = 'splunk' | 'tenable' | 'aws_security_hub' | 'crowdstrike';
type View = 'onboarding' | 'approvals';
...
function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-4">
      {/* Navigation Header */}
      <nav className="w-full max-w-4xl flex justify-between items-center mb-8 p-4 bg-slate-900/50 rounded-xl border border-slate-700 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <div className="bg-indigo-600 p-2 rounded-lg">
            <Shield size={24} className="text-white" />
          </div>
          <span className="font-bold text-xl tracking-tight">Agentic Fabric v2.0</span>
        </div>

        <div className="flex bg-slate-800 p-1 rounded-lg">
          <button
            onClick={() => setView('onboarding')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm transition-all ${view === 'onboarding' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
          >
            <Zap size={16} /> Connectors
          </button>
          <button
            onClick={() => setView('approvals')}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm transition-all ${view === 'approvals' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
          >
            <LayoutDashboard size={16} /> Approvals
          </button>
        </div>
      </nav>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 3 && (
              ...
              >
                ...
                <button onClick={resetFlow} className="w-full mb-4">
                  Connect Another Source
                </button>

                <button onClick={() => setView('approvals')} className="w-full bg-slate-700 hover:bg-slate-600">
                  Go to Dashboard
                </button>
              </motion.div>
            )}
